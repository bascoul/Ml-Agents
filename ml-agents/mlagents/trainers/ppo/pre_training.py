...
class PreTraining(object):
    ...
    def __init__(self, sess, policy_model: LearningModel, brain, parameters):
        ...
        if "demo_path" not in parameters:
            raise TrainerError("Pre-Training was set to true for {0} but no demo_path parameter was "
                               "found.".format(brain.brain_name))
        ...
        if not brain_params == brain:
            raise TrainerError("The brain parameters for {} are not compatible with the brain parameters found in "
                               "the demo file {}".format(brain.brain_name, parameters["demo_path"]))
    ...
    def _update(self, mini_batch, num_sequences):
        ...
        feed_dict = {self.policy_model.batch_size: num_sequences,
